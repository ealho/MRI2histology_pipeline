%Author: Eduardo Alho
%Date : 22/03/2017
%Uses N4 algorithm to correct MRI field inhomogenity:
...
% In Matlab, go to the directory where MRI images are (to run this script
% you have to be inside the folder with MRI images)
% name1: name of volume to be corrected Ex: ('T1.nii')
% name2: output name with extension ('corrected.nii')


function N4(root_dir, name1)


    name = strcat (root_dir,name1);
    new_name= fullfile(root_dir,(strcat('N4_corrected_',name1)));

%command = sprintf('N4BiasFieldCorrection -d 3 -i %s -x -r -w -s -c -b -t -o%s, name1, name2);
command = sprintf('N4BiasFieldCorrection -d 3 -i %s -o %s', name, new_name);
system(command)
